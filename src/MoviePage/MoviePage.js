...
const MoviePage = (props) => {
    ...
    function getStars(rating) {
        ...
        for (var i = rating; i >= 1; i--) {
            let key = i;
            output.push(
                <svg key={key} id={key} xmlns="http://www.w3.org/2000/svg" width="1.25rem" height="1.25rem" fill="currentColor" className="bi bi-star-fill" viewBox="0 0 16 16" style={{color: 'gold'}}>
        ...
        if (i === .5) {
            output.push(
                <svg key={i} id={i} xmlns="http://www.w3.org/2000/svg" width="1.25rem" height="1.25rem" fill="currentColor" className="bi bi-star-half" viewBox="0 0 16 16" style={{color: 'gold'}}>
        ...
        for (let i = (10 - rating); i >= 1; i--)
          output.push(
            <svg key={10 - i} id={10 - i} xmlns="http://www.w3.org/2000/svg" width="1.25rem" height="1.25rem" fill="currentColor" className="bi bi-star" viewBox="0 0 16 16" style={{color: 'gold'}}>
                    ...
                    <div>
                        ...
                        <div className='cast-list-wrapper'>
                            <ul className='no-style-list cast-list'>
                                {cast.map(person => {
                                    return <li>
                                        <Link className="actor-link" to={`/person/${person.id}`} state={{ person }}>
                                            <img
                                                className='actor-image'
                                                src={`https://image.tmdb.org/t/p/original/${person.profile_path}`} 
                                                alt={`${person.name}`}
                                            />
                                            <label className='actor-name'>{person.name}</label>
                                        </Link>
                                    </li>
                                })}
                            </ul>
